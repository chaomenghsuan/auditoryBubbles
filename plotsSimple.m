...
for target = 1:length(resFiles)
    ...
    for p = 1:size(res.clean,3)
        ...
        % added
        % to save the variables for the transparency plots
        %spt = strsplit(inDir,filesep);
        %fd = extractBefore(string(spt(7)), '_result');
        %outFileName = strcat('parameters', string(target), '.mat');
        %path = fullfile(spt(2), spt(3), spt(4), spt(5), spt(6), cellstr(fd), cellstr('wrong_answers'));
        %file2save = fullfile(spt(2), spt(3), spt(4), spt(5), spt(6), cellstr(fd), cellstr('wrong_answers'), cellstr(outFileName));
        %path = strcat('/', string(path));
        %if ~exist(string(path), 'dir')
        %    mkdir(char(path));
        %end
        %file2save = strcat('/', string(file2save));
        %save(file2save);
        % end
